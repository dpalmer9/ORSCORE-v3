...
current_dir.replace(protocol_folder,"")
...
def Create_Experiment():
    ...
    def SOR_Finish():
        ...
        exp_file.write("\n")
        exp_file.write("Protocol = SLRv11")
        exp_file.write("\n")
        ...
        if Condition_CSV_Create == 1:
            condition_csv_filepath = data_file_dir + folder_symbol + "Conditions.csv"
...
def Trial_Setup(Curr_Exp, Curr_Raw_Data):
    ...
    class Trial_Create():
        ...
        def var_define(self):
            ...
            if self.Condition_Csv == 1:
                ...
                self.prot_string = self.folder_symbol + 'Protocols'
                self.current_dir = self.current_dir.replace(self.prot_string,"")
                self.data_dir = self.current_dir + self.folder_symbol + "Data" + self.folder_symbol + self.Experiment_ID
                self.condition_filepath = self.data_dir + self.folder_symbol + "Conditions.csv"
    ...
    class Active_Trial():
        ...
        def trial_finish(self):
            ...
            self.current_dir.replace(self.protocol_folder, "")
